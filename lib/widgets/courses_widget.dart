    ...
    return FutureBuilder(
      ...
      builder: (ctx, snapshot) {
        ...
        var courses = List<Course>.from(
            snapshot.data?.docs.map((e) => Course.fromJson({'id': e.id, ...e.data()})).toList() ?? []);
              ...
              children: [
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: List.generate(5, (starindex) {
                                  return Icon(
                                    starindex < (courses[index].rating ?? 0) ? Icons.star : Icons.star_border,
                ...
                BlocBuilder<CartBloc, CartState>(
                  builder: (context, state) {
                    return InkWell(
                        onTap: () {
                          context.read<CartBloc>().add(AddingToCartEvent(courses[index]));
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              backgroundColor: ColorUtility.deepYellow,
                              content: Text('${courses[index].title} added to cart'),
                            ),
                          );
                        },
                        child: const Text(
